...
### 23. Docs HTML files must stay in sync with their .md counterparts
The docs directory now contains 5 HTML files with a 1:1 mapping to Markdown sources:

| HTML file              | Markdown source        | Content                      |
|------------------------|------------------------|------------------------------|
| `docs/index.html`      | (homepage, no .md)     | Landing page with card links |
| `docs/architecture.html` | `docs/architecture.md` | Architecture reference       |
| `docs/marketing.html`  | `docs/marketing.md`    | Marketing strategy           |
| `docs/roi.html`        | `docs/roi.md`          | ROI & TCO analysis           |
| `docs/benchmarks.html` | (no .md, data-driven)  | Benchmarks & analysis        |

**Rule**: Whenever any file under `docs/*.md` is edited, mirror the changes in the corresponding `docs/*.html` file in the same commit. Never let the HTML fall out of sync with the source markdown.

**Generation**: The HTML files are generated by `scripts/split_docs.py`. Do NOT regenerate from scratch — that would overwrite benchmark figures that are HTML-only. Instead make targeted edits to both `.md` and `.html` files simultaneously.

**Cross-page navigation**: Each HTML page has a sidebar nav with links to all other pages (e.g. `href="roi.html"`). If a page is renamed or added, update all 4 sidebar navs.

### 25. docs HTML pages: always use visual HTML/CSS/SVG diagrams — no ASCII art
